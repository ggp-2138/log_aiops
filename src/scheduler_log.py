...
def parse_logs(template_miner, logs):
    ...
    for log_line in logs:
        ...
        if match:
            ...
            if matched_cluster is not None:
                events.append(
                    {"timestamp": ts, "cluster_id": matched_cluster.cluster_id}
                )
    ...
    if not df.empty:
        ...
        counts = (
            df.groupby("cluster_id").resample(f"{swm}min").size().unstack(fill_value=0)
        )
        print(f"\n每{swm}分钟日志模板频次统计表：")
        # print(counts)
        return counts
...
def aggregate_anomalies(anomalies):
    ...
    for tid, tdf in df.groupby("template_id"):  # 按模板ID分组(tid)
        ...
        for gid, gdf in tdf.groupby("anomaly_interval_id"):  # 按异常区间分组(gid)
            actual_duration = (
                gdf["ts"].max() - gdf["ts"].min()
            ).total_seconds() / 60 + swm  # 实际异常持续时间
...
def analyze_root_causes(aggregation):
    ...
    # 固定根因巡检
    print("🚀 开始固定根因巡检")
    recently_root_causes = get_top3_root_cause(
        anomalous_tids, recently_detect_start, recently_detect_end
    )
...
def build_signed_url():
    ...
    sign = base64.b64encode(
        hmac.new(
            config.DINGTALK_SECRET.encode(), sign_str.encode(), hashlib.sha256
        ).digest()
    )
...
# ========== 7.钉钉推送 ==========
def send_dingtalk(aggregation, miner, root_causes, recently_root_causes, query_info):
    """发送钉钉 Markdown 告警消息"""
    # 测试可用成员
    # 适配clusters是字典结构
...
def main():
    ...
    # ========== 钉钉告警推送 ===============
    send_dingtalk(
        aggregation, template_miner, root_causes, recently_root_causes, query_info
    )
